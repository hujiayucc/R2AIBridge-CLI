...
class AIAnalyzer:
    ...
    def _build_system_prompt_strict(tool_names_hint: str, tool_required_hint: str) -> str:
        return (
            ...
            "\n"
            "0.6 r2 session_id 强规则（必须遵守）\n"
            "- `session_id` 只能来自真实工具输出（例如 `r2_open_file` 的返回）；禁止猜测/伪造/沿用不确定来源的 session_id。\n"
            "- 任何 r2_* 工具返回 `Invalid session_id` / `session invalid`：视为“可恢复”状态错误。\n"
            "  - 禁止用同一个 session_id 继续重试同一工具；这会导致连续失败。\n"
            "  - 必须先重新 `r2_open_file`（使用你已知的原始 file_path）获取新 session_id，再继续后续 r2_* 调用。\n"
            "  - 如果你不知道 file_path：必须向用户提问获取，禁止猜。\n"
            "\n"
            "当前可用工具名（运行时注入，以此为准）：\n"
            f"{tool_names_hint}\n"
            "\n"
            ...
            "- 禁止“无变化重试”：同一工具/同一参数失败后，下一轮必须先修正根因（补参/重开 session/换工具/问用户），不能原样重复刷到失败上限。\n"
            "\n"
            "========================\n"
            "2) 核心状态机（强制执行）\n"
            "========================\n"
    ...
    def _build_system_prompt_loose(tool_names_hint: str, tool_required_hint: str) -> str:
        return (
            ...
            "\n"
            "0.6 r2 session_id 强规则（必须遵守）\n"
            "- `session_id` 只能来自真实工具输出（例如 `r2_open_file` 的返回）；禁止猜测/伪造。\n"
            "- 遇到 `Invalid session_id` / `session invalid`：必须先重新 `r2_open_file` 获取新 session_id，再继续；不要用旧 session_id 重试。\n"
            "\n"
            "当前可用工具名（运行时注入，以此为准）：\n"
            f"{tool_names_hint}\n"
            "\n"
